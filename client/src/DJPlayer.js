...
import useGradualVolumeChange from './utils/useGradualVolumeChange';
...
const DJPlayer = () => {
    ...
    const { isPlaying } = useContext(CustomAudioContext);
    ...
    const changeCircles = (circle, frequency) => {
        ...
        circle.style.backgroundColor = `rgba(${dataArray[frequency] / 2
            }, 58, 183, .55)`;
    ...
    const generateBorderRadius = () => {
        ...
        return `${top}% ${100 - top}% ${bottom}% ${100 - bottom
    ...
    useGradualVolumeChange(audioRef, isPlaying ? 0.01 : 1.0, 2000);
